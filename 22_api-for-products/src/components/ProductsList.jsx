  ...
  return (
    <main className="container flex flex-col items-center w-full max-w-6xl mx-auto px-2.5">
      <div className="bg-amber-200 dark:bg-gray-700 max-w-[50%] w-full mt-3.5 h-9 flex relative">
        <img
          src={searchSvg}
          className="w-full h-full max-w-8 max-h-7 self-center absolute"
        />
        <input
          type="text"
          ...
          .map((product) => {
            return (
              <Link
                ...
                state={products}
              >
                <div className="image h-64 relative">
                  <div className="absolute right-2 flex">
                    {
                      Array.from({length:Math.floor(product.rating.rate)}).map((item,i)=>{
                        return <svg key={i}
                      className="w-4 fill-yellow-500"
                      viewBox="0 0 24 24"
                      fill="none"
                    ...
                    >
                     ...
                     {
                      Array.from({length:5-Math.floor(product.rating.rate)}).map((item,i)=>{
                        return <svg key={i}
                      className="w-4"
                      viewBox="0 0 24 24"
                      fill="none"
