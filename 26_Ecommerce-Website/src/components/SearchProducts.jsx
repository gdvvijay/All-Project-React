...
export default function SearchProduct({searchProduct,setSearchProduct}){
    ...
    return(
       <section className="all-product-section max-w-6xl w-full mx-auto">
        <div className="container mt-20 max-md:mt-10 grid grid-cols-[repeat(auto-fit,minmax(270px,1fr))] justify-items-center gap-5 px-1.5">
          {
            data.allItem.filter((filteredItem)=>filteredItem.productName.toLowerCase().includes(searchProduct.toLowerCase())).map((item)=><Link onClick={handleLink} state={item} to={`/productdetails`} key={item.id} className="item-1 flex flex-col gap-3.5 w-full  max-w-[400px]">
